...
* **测试用例（C over Rust）**：`infra/testcases/` 独立 workspace，coda（std）为框架与统一入口（banner/panic hook/C 段调用/计数/退出码都在 `coda::run_and_exit`），C 测试体经用例 crate 的一行 build.rs（coda-build 共享实现，cc crate）编入同一 musl 静态二进制（配方与 tools 同：零 rustflags + `--target <musl triple>`）；`c/` 为空时 coda-build 编入空桩 `run_c_tests` 兜住符号。C 编译器由 `cross` 以 `CC_<TRIPLE>` 注入 zig cc 包装，交叉构建无 CC 接管时 coda-build fail-fast（防 glibc/musl 混链）。宿主缺对应架构 rust-std 时显式 WARN 跳过（构建失败仍然报错）。新用例 = 复制 test-example 目录（members 走 `test-*` glob，零接线）。
