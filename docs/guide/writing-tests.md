...
2. **Rust 测试**：`src/main.rs` 的 `TESTS` 注册 `("名称", fn() -> bool)`，
   断言用 `coda::check!`（或 `pass!/fail!/skip!` 手动组合）。main 只有一行
   ——banner、panic hook、C 段调用、计数与退出码全部在 `coda::run_and_exit`：
...
VM 能力类的模块（直通、NUMA、pmem 等）仍走[组件机制](../components/overview.md)。
...
`--only` 经 kernel cmdline（`virtuoso.only=<names>`）传给 init，只跑名单内
的 `/tests` 二进制；标记协议 v1 不变（选中的照常吐全套标记）。名单零命中
（拼错名字）会直接判失败——不会出现 0/0 假绿。
