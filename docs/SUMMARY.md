# Summary

[总览](index.md)

# 入门

- [快速开始](quick-start.md)

# 架构

- [总体架构](architecture/overview.md)
- [核心 crate 设计](architecture/crates.md)
- [冻结契约](architecture/contracts.md)

# 组件

- [组件机制](components/overview.md)
- [tools_disk — VM 内工具盘](components/tools-disk.md)
- [agent — AI probe 通道](components/agent.md)
- [vfio — PCI 直通](components/vfio.md)
- [numa — 多节点拓扑](components/numa.md)
- [pmem — 持久内存](components/pmem.md)

# 使用指南

- [配置参考](guide/configuration.md)
- [preset 内核（开箱即用）](guide/kernel-preset.md)
- [编写测试用例](guide/writing-tests.md)
- [调试](guide/debugging.md)
- [运行工件与跨 run 分析](guide/artifacts.md)
- [AI 集成](guide/ai-integration.md)

# 内部机制

- [两段式引导与构建流水线](internals/boot-pipeline.md)

# 参考

- [CLI 参考](cli-reference.md)
- [Troubleshooting](troubleshooting.md)
- [贡献约定](contributing.md)
