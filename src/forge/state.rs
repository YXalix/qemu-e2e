...
pub(crate) fn read(project_root: &Path) -> anyhow::Result<Option<Current>> {
...
pub(crate) fn write(project_root: &Path, current: &Current) -> anyhow::Result<()> {
    let p = path(project_root);
    std::fs::create_dir_all(p.parent().expect("状态文件必有父目录"))?;
    std::fs::write(&p, format!("{}\n", serde_json::to_string_pretty(current)?))?;
    crate::forge::devcontainer::render(project_root, current)?;
    Ok(())
}
