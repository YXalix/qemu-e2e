...
//! - verdict.json 的 schema 单点定义在 `crate::judge::report::VerdictReport`（此处
//!   re-export）；退出码语义单点在 `crate::judge::exit`。正常收尾与 Ctrl-C 兜底
//!   共用同一构造路径。

pub mod rundir;

pub(crate) use crate::judge::report::RunMeta;
