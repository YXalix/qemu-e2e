...
pub(crate) struct TestEntry {
    pub name: String,
    pub status: crate::judge::TestStatus,
...
impl RunMeta {
    ...
    fn exit_semantics(&self) -> &'static str {
        crate::judge::exit::semantics(self.exit_code)
    }
    ...
    pub(crate) fn verdict_of(&self, audit: &Audit) -> Verdict {
        if self.build_failed {
            ...
            crate::judge::judge(self.exit_code.unwrap_or(-1), false, audit)
        }
    }
}
...
impl VerdictReport {
    ...
    pub(crate) fn build(audit: &Audit, meta: &RunMeta) -> VerdictReport {
        VerdictReport {
            ...
            started_at_utc: meta.started_ms().map(crate::time::format_utc),
